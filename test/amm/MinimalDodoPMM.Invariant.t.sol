...
contract MinimalDodoPMMHandler is Test {
    ...
    function updateOracle(uint96 rawPrice, uint64 rawAge) external {
        oracleCalls++;
        uint256 price = bound(uint256(rawPrice), 1, 1000 * ONE);
        uint256 maxAge = _min(pool.oracleMaxStaleness(), block.timestamp);
        uint256 age = maxAge == 0 ? 0 : bound(uint256(rawAge), 0, maxAge);
        oracle.setPriceWithTimestamp(price, block.timestamp - age);
    }
...
contract MinimalDodoPMMInvariantTest is AMMTestBase {
    ...
    function setUp() public override {
        ...
        handler = new MinimalDodoPMMHandler(
            pool, base, quote, oracle, address(this), supervisor, maintainer, taxRecipient, actors
        );
        ...
        selectors[5] = handler.updateOracle.selector;
    ...
    function invariantTrackedBalancesNeverExceedActualBalances() public view {
        _assertTrackedBalancesAtMostActual();
    }

    function invariantEmptyPoolStateIsReset() public view {
        if (pool.totalSupply() == 0) {
            _assertEmptyPoolState();
        }
    }

    function invariantNonEmptyPoolHasTrackedReserves() public view {
        if (pool.totalSupply() > 0) {
    ...
    function invariantEffectiveKWithinConfiguredBoundsWhenOracleValid() public view {
    ...
    function invariantFeeAndKParametersStayWithinBounds() public view {
        assertGt(pool.k(), 0);
        assertLt(pool.k(), ONE);
    ...
    function invariantActorLpBalancesSumToTotalSupply() public view {
        assertEq(_sumLpBalances(_actorSet()), pool.totalSupply());
    }

    function invariantQueriesAreBoundedWhenTheySucceed() public view {
        if (pool.totalSupply() == 0) {
            return;
        }
